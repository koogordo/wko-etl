...
const jsonFilePaths = dir.files('wko-new/json', {sync:true})
jsonFilePaths.forEach(path => {
    ...
    const writeStream = require('fs').createWriteStream(`wko-new/props-list/${formNode.name.split(' ').join('')}.txt`);
