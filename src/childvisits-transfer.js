...
familiesDB.allDocs({ include_docs: true }).then(payload => {
    ...
    const childIdMaps = families.map(fam => { return fam.idMap }).flat().filter(idPair => { return idPair.newId.includes('child') });
    const dataPromises = [];
    let c = 0;
    for (const client of childIdMaps) {
        // const client = adultIdMaps.filter(idmap => { return idmap.oldId = 1566 })[0];
        console.log(client);
        // ANCHOR --begin going through clients
        let putVisits = [];
        dataPromises.push(sql.query(`SELECT DISTINCT * FROM childvisits cv1 where cv1.ClientID = ?`, [client.oldId]).then(rows => { return rows }));
        Promise.all(dataPromises).then(([sqlChildVisits]) => {
            // ANCHOR --begin data promises block
            let c = 0;
            for (const sqlVisit of sqlChildVisits) {
                ...
                const template = JSON.parse(JSON.stringify(cvt));
                ...
                for (const key in cvk) {
                    const oldDbMap = cvk[key];
