...
const idMaps = (families) => {
    return families.filter(fam => {
        ...
        return idPair.newId.includes('child')
    });
}

const clientVisits = (client) => {
    return sql.query(`SELECT DISTINCT * FROM childvisits cv1 WHERE cv1.ClientID = ?`, [client.oldId]).then(rows => { return rows }).catch(err => console.log(err));
}
...
const main = () => {
    families().then(families => {
        const childIdMaps = idMaps(families);
        const clientPromises = childIdMaps.map(buildClientVisits);
